...
void SIMD_ADD_16(float* x, float* y) {
    ...
    if(LANG_NUM == 0) {
      FILE* fp0 = fopen("./IR/C_SIMD-to-IR_APIM_ISSAC.trace","a");
    	...
    	ADDR(y); 
    	fprintf(fp0, " y: tile %d, mac_unit %d, x-bar %d, add6, 2)\n", ti, rb, xb); 
...
float MAC_16(float* v_a, float* v_b) {
    ...
    if(LANG_NUM == 0) {
     ...
     fprintf(fp0, "apim.vector_acc(b: tile %d, mac_unit %d, x-bar %d, add, 16)\n", ti, rb, xb);
    	fprintf(fp0, "apim.store(b: tile %d, mac_unit %d, x-bar %d, add)\n", ti, rb, xb); 
      fclose(fp0);
     }
     if(LANG_NUM == 1) {
      FILE* fp0 = fopen("./IR/OpenMP_SIMD-to-IR_APIM_ISSAC.trace","a");
    ...
    if(LANG_NUM == 0) {
     ...
     fprintf(fp1, "apim.vector_acc(b: chip %d, reram_bank %d, x-bar %d, add, 16)\n", ti, rb, xb);
    	fprintf(fp1, "apim.store(b: chip %d, reram_bank %d, x-bar %d, add)\n", ti, rb, xb); 
      fclose(fp1);
     }
     if(LANG_NUM == 1) {
      FILE* fp1 = fopen("./IR/OpenMP_SIMD-to-IR_APIM_PRIME.trace","a");
    ...
    if(LANG_NUM == 0) {
      ...
      fprintf(fp2, "b: ch %d ra %d bg %d ba %d ro %d co %d, add, 16, mul)\n", ch, ra, bg, ba, ro, co);
		  ...
		  fprintf(fp2, "dpim.store(b: ch %d ra %d bg %d ba %d ro %d co %d, add)\n", ch, ra, bg, ba, ro, co);
      fclose(fp2); 
     }
     if(LANG_NUM == 1) {
      FILE* fp2 = fopen("./IR/OpenMP_SIMD-to-IR_DPIM_Newton.trace","a");
    ...
    if(LANG_NUM == 0) {
      ...
      fprintf(fp3, "b: ch %d ra %d bg %d ba %d ro %d co %d, add, 16, mul)\n", ch, ra, bg, ba, ro, co);
		  ...
		  fprintf(fp3, "dpim.store(b: ch %d ra %d bg %d ba %d ro %d co %d, add)\n", ch, ra, bg, ba, ro, co);
      fclose(fp3); 
   }
   if(LANG_NUM == 1) {
      FILE* fp3 = fopen("./IR/OpenMP_SIMD-to-IR_DPIM_HBM-PIM.trace","a");
  ...
  float VEC_ACC(float *x, int width) {
	int temp = width;
	  ...
	  while(temp>1){
        ...
        if(LANG_NUM == 0) {
          ...
          fprintf(fp6, "apim.store(x: tile %d, mac_unit %d, x-bar %d, add)\n", ti, rb, xb); 
          fclose(fp6); 
        }
        if(LANG_NUM == 1) {
        ...
        if(LANG_NUM == 0) {
          ...
          fprintf(fp7, "apim.store(x: chip %d, reram_bank %d, x-bar %d, add)\n", ti, rb, xb); 
          fclose(fp7);
        }	
        if(LANG_NUM == 1) {
	...
	while(temp>1){
        ...
        if(LANG_NUM == 0) {
          ...
          ADDR(x);
	        fprintf(fp2, "dpim.store(x: ch %d ra %d bg %d ba %d ro %d co %d, add)\n", ch, ra, bg, ba, ro, co); 
          fclose(fp2); 
         }
         if(LANG_NUM == 1) {
          FILE* fp2 = fopen("./IR/OpenMP_SIMD-to-IR_DPIM_Newton.trace","a");
        ...
        if(LANG_NUM == 0) {
          ...
          ADDR(x);
	        fprintf(fp3, "dpim.store(x: ch %d ra %d bg %d ba %d ro %d co %d, add)\n", ch, ra, bg, ba, ro, co); 
          fclose(fp3); 
       }
       if(LANG_NUM == 1) {
          FILE* fp3 = fopen("./IR/OpenMP_SIMD-to-IR_DPIM_HBM-PIM.trace","a");
