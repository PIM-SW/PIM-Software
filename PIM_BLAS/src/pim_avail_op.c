...
void ADDR(float* x) {
  int addr = (int) x;
...
#ifdef DPIM
  #ifdef MAP_TYPE1 // Row-wise (Newton)
    bg_pos = 6, ba_pos = 8, co_pos = 10, ch_pos = 16, ra_pos = 17, ro_pos = 18;
    bg_mask = 3, ba_mask = 3,  co_mask = 63, ch_mask = 1, ra_mask = 1, ro_mask = 16384;
  #endif
  #ifdef MAP_TYPE2 // Row-wise (HBM-PIM)
    bg_pos = 6, ba_pos = 8, co_pos = 10, ch_pos = 16, ra_pos = 17, ro_pos = 18;
    bg_mask = 3, ba_mask = 3,  co_mask = 63, ch_mask = 1, ra_mask = 1, ro_mask = 16384;
  #endif 
#endif
    ...
    ch_pos = 6, ra_pos = 7, bg_pos = 8, ba_pos = 10, co_pos = 12, ro_pos = 18;
    ch_mask = 1, ra_mask = 1, bg_mask = 3, ba_mask = 3, co_mask = 63, ro_mask = 16384;
  #endif 
#endif
...
void SIMD_ADD_16(float* x, float* y) {
    ...
    if(LANG_NUM == 0) {
      FILE* fp0 = fopen("./IR/C_SIMD-to-IR_APIM_ISSAC.trace","a");
    	...
    	ADDR(y); 
    	fprintf(fp0, " y: tile %d, mac_unit %d, x-bar %d, 16, 2)\n", ti, rb, xb); 
    	fprintf(fp0, "apim.store(y: tile %d, mac_unit %d, x-bar %d, 16)\n", ti, rb, xb); 
      fclose(fp0);
     }
     if(LANG_NUM == 1) {
    ...
    if(LANG_NUM == 0) {
      FILE* fp1 = fopen("./IR/C_SIMD-to-IR_APIM_PRIME.trace","a");
    	ADDR(x);
    	fprintf(fp1, "apim.load(a: chip %d, reram_bank %d, x-bar %d, reg: %d, 16)\n", ti, rb, xb, reg); 
    	fprintf(fp1, "apim.vector(reg: %d, ", reg); 
    	ADDR(y);
    	fprintf(fp1, " y: chip %d, reram_bank %d, x-bar %d, 16, 2)\n", ti, rb, xb); 
    	fprintf(fp1, "apim.store(y: chip %d, reram_bank %d, x-bar %d, 16)\n", ti, rb, xb);
      fclose(fp1);
     } 
     if(LANG_NUM == 1) {
    ...
    if(LANG_NUM == 0) {
      FILE* fp2 = fopen("./IR/C_SIMD-to-IR_DPIM_Newton.trace","a");
		  ADDR(x);
      fprintf(fp2, "dpim.set(a: ch %d ra %d bg %d ba %d ro %d co %d, reg: %d, 16)\n", ch, ra, bg, ba, ro, co, reg);
	    fprintf(fp2, "dpim.vector(reg: %d, ", reg); 
		  ADDR(y);
			fprintf(fp2, " y: ch %d ra %d bg %d ba %d ro %d co %d, 16, mul)\n", ch, ra, bg, ba, ro, co);
			fprintf(fp2, "dpim.store(y: ch %d ra %d bg %d ba %d ro %d co %d, 16)\n", ch, ra, bg, ba, ro, co);
      fclose(fp2); 
     }
     if(LANG_NUM == 1) {
    ...
    if(LANG_NUM == 0) {
      FILE* fp3 = fopen("./IR/C_SIMD-to-IR_DPIM_HBM-PIM.trace","a");
		  ADDR(x);
      fprintf(fp3, "dpim.set(a: ch %d ra %d bg %d ba %d ro %d co %d, reg: %d, 16)\n", ch, ra, bg, ba, ro, co, reg);
	    fprintf(fp3, "dpim.vector(reg: %d, ", reg); 
		  ADDR(y);
			fprintf(fp3, " y: ch %d ra %d bg %d ba %d ro %d co %d, 16, mul)\n", ch, ra, bg, ba, ro, co);
			fprintf(fp3, "dpim.store(y: ch %d ra %d bg %d ba %d ro %d co %d, 16)\n", ch, ra, bg, ba, ro, co);
      fclose(fp3);
   }
   if(LANG_NUM == 1) {
  ...
  if(LANG_NUM == 0) {
    ...
    ADDR(x);     
     ...
     ADDR(y);
    fprintf(fp4, " y: ch %d ra %d bg %d ba %d ro %d co %d, 16, 0)\n", ch, ra, bg, ba, ro, co);
    fclose(fp4);
   }
  if(LANG_NUM == 1) {
  ...
  if(LANG_NUM == 0) {
    ...
    ADDR(x);     
     ...
     ADDR(y);
    fprintf(fp5, " y: ch %d ra %d bg %d ba %d ro %d co %d, 16, 0)\n", ch, ra, bg, ba, ro, co);
    fclose(fp5);
   }
   if(LANG_NUM == 1) {
...
void SIMD_SUB_16(float* x, float* y) {
    ...
    ADDR(y); 
    printf(" y: ch %d ra %d bg %d ba %d ro %d co %d, 16, 1)\n", ch, ra, bg, ba, ro, co); 
#endif
    for(int i=0; i<16; i++) {
        y[i] = x[i] - y[i];
    }
...
void SIMD_MUL_16(float* x, float* y) {
    ...
    ADDR(x);   
	  printf("pnm.vector_op(x: ch %d ra %d bg %d ba %d ro %d co %d, ", ch, ra, bg, ba, ro, co);
    ADDR(y); 
    printf(" y: ch %d ra %d bg %d ba %d ro %d co %d, 16, 2)\n", ch, ra, bg, ba, ro, co); 
#endif
    for(int i=0; i<16; i++) {
        y[i] = x[i] * y[i];
    }
...
void SIMD_DIV_16(float* x, float* y) {
    ...
    ADDR(x);   
	  printf("pnm.vector_op(x: ch %d ra %d bg %d ba %d ro %d co %d, ", ch, ra, bg, ba, ro, co);
    ADDR(y); 
    printf(" y: ch %d ra %d bg %d ba %d ro %d co %d, 16, 3)\n", ch, ra, bg, ba, ro, co); 
#endif
    for(int i=0; i<16; i++) {
        y[i] = x[i] / y[i];
    }
...
void SIMD_SCAL_ADD_16(float a, float* y) {
    ...
    printf(" y: ch %d ra %d bg %d ba %d ro %d co %d, 16, 0)\n", ch, ra, bg, ba, ro, co); 
#endif
    for(int i=0; i<16; i++) {
        y[i] = y[i] + a;
    }
...
void SIMD_SCAL_MUL_16(float a, float* y) {
    ...
    if(LANG_NUM == 0) {
      FILE* fp2 = fopen("./IR/C_SIMD-to-IR_DPIM_Newton.trace","a");
		  ADDR(&a);
      fprintf(fp2, "dpim.load(a: ch %d ra %d bg %d ba %d ro %d co %d, reg: %d, 16)\n", ch, ra, bg, ba, ro, co, reg);
    ...
    if(LANG_NUM == 0) {
      FILE* fp3 = fopen("./IR/C_SIMD-to-IR_DPIM_HBM-PIM.trace","a");
		  ADDR(&a);
      fprintf(fp3, "dpim.load(a: ch %d ra %d bg %d ba %d ro %d co %d, reg: %d, 16)\n", ch, ra, bg, ba, ro, co, reg);
  ...
  if(LANG_NUM == 0) {
    ...
    fprintf(fp4, " y: ch %d ra %d bg %d ba %d ro %d co %d, 16, 2)\n", ch, ra, bg, ba, ro, co);
    fclose(fp4);
   }
  if(LANG_NUM == 1) {
    FILE* fp4 = fopen("./IR/OpenMP_SIMD-to-IR_PNM_RecNMP.trace","a");
  ...
  if(LANG_NUM == 0) {
    ...
    fprintf(fp5, " y: ch %d ra %d bg %d ba %d ro %d co %d, 16, 2)\n", ch, ra, bg, ba, ro, co);
    fclose(fp5);
   }
   if(LANG_NUM == 1) {
    FILE* fp5 = fopen("./IR/OpenMP_SIMD-to-IR_PNM_TensorDIMM.trace","a");
...
float MAC_16(float* v_a, float* v_b) {
    ...
    if(LANG_NUM == 0) {
      FILE* fp0 = fopen("./IR/C_SIMD-to-IR_APIM_ISSAC.trace","a");
    	...
    	fprintf(fp0, "apim.vector(reg: %d, ", reg); 
    	ADDR(v_b); 
    	fprintf(fp0, " b: tile %d, mac_unit %d, x-bar %d, 16, 2)\n", ti, rb, xb); 
     fprintf(fp0, "apim.vector_acc(b: tile %d, mac_unit %d, x-bar %d, 16)\n", ti, rb, xb);
    ...
    if(LANG_NUM == 0) {
      FILE* fp1 = fopen("./IR/C_SIMD-to-IR_APIM_PRIME.trace","a");
    	...
    	fprintf(fp1, "apim.vector(reg: %d, ", reg); 
    	ADDR(v_b); 
    	fprintf(fp1, " b: chip %d, reram_bank %d, x-bar %d, 16, 2)\n", ti, rb, xb); 
     fprintf(fp1, "apim.vector_acc(b: chip %d, reram_bank %d, x-bar %d, 16)\n", ti, rb, xb);
    ...
    if(LANG_NUM == 0) {
      ...
      fprintf(fp2, "dpim.load(a: ch %d ra %d bg %d ba %d ro %d co %d, reg: %d, 16)\n", ch, ra, bg, ba, ro, co, reg);
	    fprintf(fp2, "dpim.vector(reg: %d, ", reg); 
		  ADDR(v_b);
      fprintf(fp2, "b: ch %d ra %d bg %d ba %d ro %d co %d, 16, 2)\n", ch, ra, bg, ba, ro, co);
		  fprintf(fp2, "dpim.vector_acc(b: ch %d ra %d bg %d ba %d ro %d co %d, 16)\n", ch, ra, bg, ba, ro, co);
    ...
    if(LANG_NUM == 0) {
      ...
      fprintf(fp3, "dpim.load(a: ch %d ra %d bg %d ba %d ro %d co %d, reg: %d, 16)\n", ch, ra, bg, ba, ro, co, reg);
	    fprintf(fp3, "dpim.vector(reg: %d, ", reg); 
		  ADDR(v_b);
      fprintf(fp3, "b: ch %d ra %d bg %d ba %d ro %d co %d, 16, 2)\n", ch, ra, bg, ba, ro, co);
		  fprintf(fp3, "dpim.vector_acc(b: ch %d ra %d bg %d ba %d ro %d co %d, 16)\n", ch, ra, bg, ba, ro, co);
  ...
  if(LANG_NUM == 0) {
    ...
    ADDR(v_b); 
    fprintf(fp4, " b: ch %d ra %d bg %d ba %d ro %d co %d, 16, 2)\n", ch, ra, bg, ba, ro, co); 
    fclose(fp4);
   }
  if(LANG_NUM == 1) {
    FILE* fp4 = fopen("./IR/OpenMP_SIMD-to-IR_PNM_RecNMP.trace","a");
  ...
  if(LANG_NUM == 0) {
    ...
    ADDR(v_b); 
    fprintf(fp5, " b: ch %d ra %d bg %d ba %d ro %d co %d, 16, 2)\n", ch, ra, bg, ba, ro, co); 
    fclose(fp5);
   }
   if(LANG_NUM == 1) {
    FILE* fp5 = fopen("./IR/OpenMP_SIMD-to-IR_PNM_TensorDIMM.trace","a");
  ...
  float VEC_ACC(float *x, int width) {
	int temp = width;
	  ...
	  while(temp>1){
		...
		#ifdef MAP_TYPE1
			  ADDR(x+i);
				printf("apim.load(a: tile %d, mac_unit %d, x-bar %d, reg: %d, %d)\n", ti, rb, xb, reg,APIM_SIMD_SIZE);
				printf("apim.vector_acc(a: tile %d, mac_unit %d, x-bar %d, %d)\n", ti, rb, xb,APIM_SIMD_SIZE); 
	   #endif
	   #ifdef MAP_TYPE2
			  ADDR(x+i);
				printf("apim.load(x: chip %d, reram_bank %d, x-bar %d, reg:%d ,%d)\n", ti, rb, xb,reg ,APIM_SIMD_SIZE); 
				printf("apim.vector_acc(x: chip %d, reram_bank %d, x-bar %d, %d)\n", ti, rb, xb,APIM_SIMD_SIZE); 
	 	 #endif
    }
		temp/=APIM_SIMD_SIZE;
	}
  	#ifdef MAP_TYPE1
  	  ADDR(x);
  		printf("apim.store(x: tile %d, mac_unit %d, x-bar %d, 1)\n", ti, rb, xb); 
  	#endif
  	#ifdef MAP_TYPE2
  	  ADDR(x);
  		printf("apim.store(x: chip %d, reram_bank %d, x-bar %d, 1)\n", ti, rb, xb); 
  	#endif
 #endif
	...
	while(temp>1){
		for(int i=0; i<temp; i+=DPIM_SIMD_SIZE) {
			ADDR(x+i);
			printf("dpim.vector_acc(x: ch %d ra %d bg %d ba %d ro %d co %d, %d)\n", ch, ra, bg, ba, ro, co,DPIM_SIMD_SIZE);
		}
		temp/=DPIM_SIMD_SIZE;
	}
	ADDR(x);
	printf("dpim.store(x: ch %d ra %d bg %d ba %d ro %d co %d, 1)\n", ch, ra, bg, ba, ro, co); 
#endif
#ifdef PNM
	while(temp>1){
		for(int i=0; i<temp; i+=PNM_SIMD_SIZE){
      ADDR(x+i);
      printf("pnm.vector_acc(x: ch %d ra %d bg %d ba %d ro %d co %d, %d)\n", ch, ra, bg, ba, ro, co, PNM_SIMD_SIZE); 
		}
		temp/=PNM_SIMD_SIZE;
	}
#endif
...
void VEC_ADD(float *x, float *y, int width) {
	...
	while(temp>1){
		for(int i=0; i<temp; i+=APIM_SIMD_SIZE){
		 #ifdef MAP_TYPE1
		  ADDR(x+i);
      printf("apim.load(x: tile %d, mac_unit %d, x-bar %d, reg: %d, 16)\n", ti, rb, xb, reg);
			printf("apim.vector(reg: %d, ", reg); 
		  ADDR(y+i);
			printf("y: tile %d, mac_unit %d, x-bar %d, %d, 0)\n", ti, rb, xb,APIM_SIMD_SIZE);
			printf("apim.store(x: tile %d, mac_unit %d, x-bar %d, %d)\n", ti, rb, xb,APIM_SIMD_SIZE); 
		 #endif
		 #ifdef MAP_TYPE2
			ADDR(x+i);
			printf("apim.load(x: chip %d, reram_bank %d, x-bar %d, reg: %d, %d)\n", ti, rb, xb, reg,APIM_SIMD_SIZE); 
			printf("apim.vector(reg: %d, ", reg); 
			ADDR(y+i);
			printf("y: chip %d, reram_bank %d, x-bar %d, %d, 0)\n", ti, rb, xb,APIM_SIMD_SIZE);
			printf("apim.store(x: chip %d, reram_bank %d, x-bar %d, %d)\n", ti, rb, xb,APIM_SIMD_SIZE); 
		 #endif
		}
		temp/=APIM_SIMD_SIZE;
	}
	...
	while(temp>1){
		for(int i=0; i<temp; i+=DPIM_SIMD_SIZE){
			ADDR(x);
			printf("dpim.load(x: ch %d ra %d bg %d ba %d ro %d co %d, reg: %d, %d)\n", ch, ra, bg, ba, ro, co, reg, DPIM_SIMD_SIZE);
			printf("dpim.vector_comp(reg: %d, ", reg); 
			ADDR(y+i);
			printf(" y: ch %d ra %d bg %d ba %d ro %d co %d, %d, add)\n", ch, ra, bg, ba, ro, co,DPIM_SIMD_SIZE);
			printf("dpim.store(y: ch %d ra %d bg %d ba %d ro %d co %d, %d)\n", ch, ra, bg, ba, ro, co,DPIM_SIMD_SIZE);
		}
		temp/=DPIM_SIMD_SIZE;
	}
#endif
	...
	while(temp>1){
		for(int i=0; i<temp; i+=PNM_SIMD_SIZE){
      ADDR(x+i);
      printf("pnm.vector_acc(x: ch %d ra %d bg %d ba %d ro %d co %d, ", ch, ra, bg, ba, ro, co);
      ADDR(y+i);
      printf(" y: ch %d ra %d bg %d ba %d ro %d co %d, %d, 0)\n", ch, ra, bg, ba, ro, co, PNM_SIMD_SIZE);
		}
		temp/=PNM_SIMD_SIZE;
	}
#endif
...
void VEC_MUL(float *x, float *y, int width) {
	...
	while(temp>1){
      ...
      if(LANG_NUM == 0) {
        ...
        ADDR(y+i);
				fprintf(fp6, " y: tile %d, mac_unit %d, x-bar %d, %d, 2)\n", ti, rb, xb,APIM_SIMD_SIZE);
      ...
      if(LANG_NUM == 0) {
        ...
        ADDR(y+i);
				fprintf(fp7, " y: tile %d, mac_unit %d, x-bar %d, %d, 2)\n", ti, rb, xb,APIM_SIMD_SIZE);
  ...
  temp = width;
    ...
    if(LANG_NUM == 0) {
      ...
      fprintf(fp2, "dpim.load(x: ch %d ra %d bg %d ba %d ro %d co %d, reg: %d, %d)\n", ch, ra, bg, ba, ro, co, reg, DPIM_SIMD_SIZE);
	    fprintf(fp2, "dpim.vector(reg: %d, ", reg); 
		  ADDR(y+i);
      fprintf(fp2, "y: ch %d ra %d bg %d ba %d ro %d co %d, 16, 2)\n", ch, ra, bg, ba, ro, co);
    ...
    if(LANG_NUM == 0) {
      ...
      fprintf(fp3, "dpim.load(x: ch %d ra %d bg %d ba %d ro %d co %d, reg: %d, %d)\n", ch, ra, bg, ba, ro, co, reg, DPIM_SIMD_SIZE);
	    fprintf(fp3, "dpim.vector(reg: %d, ", reg); 
		  ADDR(y+i);
      fprintf(fp3, "y: ch %d ra %d bg %d ba %d ro %d co %d, 16, 2)\n", ch, ra, bg, ba, ro, co);
 ...
 temp = width;
	...
	while(temp>1){
      ...
      if(LANG_NUM == 0) {
        ...
        ADDR(x+i);
        fprintf(fp10, "pnm.vector(x: ch %d ra %d bg %d ba %d ro %d co %d, %d, ", ch, ra, bg, ba, ro, co);
        ADDR(y+i);
        fprintf(fp10, " y: ch %d ra %d bg %d ba %d ro %d co %d, %d, 2)\n", ch, ra, bg, ba, ro, co, PNM_SIMD_SIZE);
        fclose(fp10);
      }
      if(LANG_NUM == 1) {
      ...
      if(LANG_NUM == 0) {
        ...
        ADDR(x+i);
        fprintf(fp11, "pnm.vector(x: ch %d ra %d bg %d ba %d ro %d co %d, %d, ", ch, ra, bg, ba, ro, co);
        ADDR(y+i);
        fprintf(fp11, " y: ch %d ra %d bg %d ba %d ro %d co %d, %d, 2)\n", ch, ra, bg, ba, ro, co, PNM_SIMD_SIZE);
        fclose(fp11);
      }
      if(LANG_NUM == 1) {
	...
	while(temp>1){
      ...
      if(LANG_NUM == 0) {
        FILE* fp6 = fopen("./IR/C_Func-to-IR_APIM_ISSAC.trace","a");
				...
				fprintf(fp6, "apim.vector_imm(%f, %p, %d, 0)\n", imm, x+i, APIM_SIMD_SIZE);
      ...
      if(LANG_NUM == 0) {
        FILE* fp7 = fopen("./IR/C_Func-to-IR_APIM_PRIME.trace","a");
				...
				fprintf(fp7, "apim.vector_imm(%f, %p, %d, 0)\n", imm, x+i, APIM_SIMD_SIZE);
 ...
 temp = width;
	...
	while(temp>1){
      ...
      if(LANG_NUM == 0) {
        FILE* fp10 = fopen("./IR/C_Func-to-IR_PNM_RecNMP.trace","a");
        ADDR(x+i);
        fprintf(fp10, "pnm.vector_imm(%f, x: ch %d ra %d bg %d ba %d ro %d co %d, %d, 2)\n", imm, ch, ra, bg, ba, ro, co, PNM_SIMD_SIZE);
        fclose(fp10);
      }
      if(LANG_NUM == 1) {
        FILE* fp10 = fopen("./IR/OpenMP_Func-to-IR_PNM_RecNMP.trace","a");
      ...
      if(LANG_NUM == 0) {
        FILE* fp11 = fopen("./IR/C_Func-to-IR_PNM_TeonsorDIMM.trace","a");
        ADDR(x+i);
        fprintf(fp11, "pnm.vector_imm(%f, x: ch %d ra %d bg %d ba %d ro %d co %d, %d, 2)\n", imm, ch, ra, bg, ba, ro, co, PNM_SIMD_SIZE);
        fclose(fp11);
      }
      if(LANG_NUM == 1) {
        FILE* fp11 = fopen("./IR/OpenMP_Func-to-IR_PNM_TeonsorDIMM.trace","a");
	...
	while(temp>1){
      ...
      if(LANG_NUM == 0) {
        FILE* fp6 = fopen("./IR/C_Func-to-IR_APIM_ISSAC.trace","a");
			  ADDR(x+i);
				fprintf(fp6, "apim.load(x: tile %d, mac_unit %d, x-bar %d, reg: %d, %d)\n", ti, rb, xb, reg,APIM_SIMD_SIZE);
				fprintf(fp6, "apim.vector(imm: %f, x: tile %d, mac_unit %d, x-bar %d, %d, 2)\n", imm, ti, rb, xb,APIM_SIMD_SIZE);	
				fprintf(fp6, "apim.store(x: tile %d, mac_unit %d, x-bar %d, %d)\n", ti, rb, xb, APIM_SIMD_SIZE); 		
        fclose(fp6); 
      }
      if(LANG_NUM == 1) {
      ...
      if(LANG_NUM == 0) {
        FILE* fp7 = fopen("./IR/C_Func-to-IR_APIM_PRIME.trace","a");
 			  ADDR(x+i);
				fprintf(fp7, "apim.load(x: chip %d, reram_bank %d, x-bar %d, reg: %d, %d)\n", ti, rb, xb, reg,APIM_SIMD_SIZE);
				fprintf(fp7, "apim.vector(imm: %f, x: chip %d, reram_bank %d, x-bar %d, %d,2)\n", imm, ti, rb, xb, APIM_SIMD_SIZE); 
...
void MV_MUL(float *x, float *A, float *y, int N, int M) {
	...
	for(int i=0; i<n3; i++){
      ...
      if(LANG_NUM == 0) {
        ...
        fprintf(fp10, " x: ch %d ra %d bg %d ba %d ro %d co %d, %d, 2)\n", ch, ra, bg, ba, ro, co, PNM_SIMD_SIZE);
        fclose(fp10);
      }
      if(LANG_NUM == 1) {
        FILE* fp10 = fopen("./IR/OpenMP_Func-to-IR_PNM_RecNMP.trace","a");
      ...
      if(LANG_NUM == 0) {
        ...
        ADDR(x+i*PNM_SIMD_SIZE);
        fprintf(fp11, " x: ch %d ra %d bg %d ba %d ro %d co %d, %d, 2)\n", ch, ra, bg, ba, ro, co, PNM_SIMD_SIZE);
        fclose(fp11);
      }
      if(LANG_NUM == 1) {
        FILE* fp11 = fopen("./IR/OpenMP_Func-to-IR_PNM_TeonsorDIMM.trace","a");
